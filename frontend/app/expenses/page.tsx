...
export default function ExpensesPage() {
  ...
  return (
    <>
      <WorkspaceNavigation />
      <main className="min-h-screen bg-[#F6F8FB] px-4 py-6 text-[#172B4D] sm:px-8">
        <div className="mx-auto max-w-6xl">
          <header className="flex flex-wrap items-end justify-between gap-4 border-b border-[#172B4D]/10 pb-6">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[.2em] text-[#2563EB]">Finance desk</p>
              <h1 className="mt-2 text-4xl font-semibold">Expenses</h1>
              <p className="mt-2 text-sm text-[#172B4D]/55">Track operational expenses through controlled submission and approval.</p>
            </div>
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => setShowForm((visible) => !visible)} className="flex items-center gap-2 bg-[#172B4D] px-4 py-2 text-xs font-semibold uppercase text-white">
          ...
          {showForm && (
            <form onSubmit={createExpense} className="mt-7 grid gap-4 bg-[#172B4D] p-5 text-white sm:grid-cols-2">
              <label className="block text-xs font-semibold uppercase tracking-[0.12em] text-white/75">
                Category
                <select required aria-label="Expense category" value={form.categoryId} onChange={(event) => setForm({ ...form, categoryId: event.target.value })} className="mt-2 h-11 w-full bg-white px-3 text-sm font-normal normal-case tracking-normal text-[#172B4D]">
              ...
              <label className="block text-xs font-semibold uppercase tracking-[0.12em] text-white/75">
                Payment method
                <select required aria-label="Expense payment method" value={form.paymentMethodId} onChange={(event) => setForm({ ...form, paymentMethodId: event.target.value })} className="mt-2 h-11 w-full bg-white px-3 text-sm font-normal normal-case tracking-normal text-[#172B4D]">
              ...
              <label className="block text-xs font-semibold uppercase tracking-[0.12em] text-white/75 sm:col-span-2">
                Description
                <input required aria-label="Expense description" placeholder="What was this expense for?" value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} className="mt-2 h-11 w-full bg-white px-3 text-sm font-normal normal-case tracking-normal text-[#172B4D] placeholder:text-[#172B4D]/50" />
              </label>
              <label className="block text-xs font-semibold uppercase tracking-[0.12em] text-white/75">
                Amount (TSh)
                <input required min="0.01" step="0.01" type="number" aria-label="Expense amount" placeholder="0.00" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} className="mt-2 h-11 w-full bg-white px-3 text-sm font-normal normal-case tracking-normal text-[#172B4D] placeholder:text-[#172B4D]/50" />
              </label>
              <button type="submit" className="h-11 self-end bg-[#D4A72C] px-4 text-xs font-semibold uppercase text-[#172B4D]">Create expense</button>
            </form>
          )}
