...
export default function DashboardPage() {
  ...
  const pulseWeeks = Array.from({ length: 5 }, (_, index) => {
    ...
    return {
      ...
      total,
      days: week.filter((item) => item.sales > 0).length,
    };
  ...
  const peakDay = trend.reduce(
    ...
    trend[0] ?? { date: "-", sales: 0 },
  );
  const recentHalf = trend
    .slice(Math.ceil(trend.length / 2))
    .reduce((sum, item) => sum + item.sales, 0);
  const earlierHalf = trend
    .slice(0, Math.ceil(trend.length / 2))
    .reduce((sum, item) => sum + item.sales, 0);
  const pulseChange = earlierHalf
    ? ((recentHalf - earlierHalf) / earlierHalf) * 100
    : recentHalf
      ? 100
      : 0;
  const cards = [
          ...
          <section className="mt-8 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {cards.map(({ label, value, note, icon: Icon, accent }) => (
              <KpiCard
                ...
                note={note}
                icon={Icon}
                accent={accent as "amber" | "teal" | "gold" | "terracotta"}
              />
            ))}
          </section>
            ...
            <article className="overflow-hidden rounded-2xl bg-[#172B4D] p-6 text-[#F6F8FB] shadow-[0_18px_50px_rgba(23,43,77,0.18)] sm:p-8">
              ...
              >
                ...
                >
                  ...
                  <path
                    d={`M ${trend.map((item, index) => `${(index / Math.max(trend.length - 1, 1)) * 500},${174 - (item.sales / maxSales) * 150}`).join(" L ")} L 500,180 L 0,180 Z`}
                    fill="url(#pulseFill)"
                  />
                  <path
