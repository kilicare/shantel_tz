...
export default function RefundsPage() {
  ...
  return <><WorkspaceNavigation /><main className="min-h-screen bg-[#F6F8FB] px-4 py-5 text-[#172B4D] sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto max-w-6xl"><header className="flex flex-col justify-between gap-5 border-b border-[#172B4D]/12 pb-7 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#2563EB]">Finance desk</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.05em]">Refunds with a trace.</h1><p className="mt-2 text-sm text-[#172B4D]/55">Return funds against a recorded customer payment.</p></div><button type="button" onClick={() => void loadData()} className="flex items-center gap-2 border border-[#172B4D]/15 px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.12em]"><RefreshCw size={15} /> Refresh</button></header>{error && <div role="alert" className="mt-6 flex items-center gap-3 border border-[#2563EB]/30 bg-[#2563EB]/8 px-4 py-3 text-sm text-[#5B3A0F]"><AlertCircle size={18} /> {error}</div>}{message && <div role="status" className="mt-6 border border-[#16805C]/30 bg-[#16805C]/10 px-4 py-3 text-sm text-[#16805C]">{message}</div>}<section className="mt-8 bg-white p-6"><h2 className="text-xl font-semibold">New refund</h2><div className="mt-4 grid gap-3 sm:grid-cols-4"><select aria-label="Original payment" value={paymentId} onChange={(event) => setPaymentId(event.target.value)} className="border border-[#172B4D]/15 bg-white px-3 py-2.5 text-sm"><option value="">Select payment</option>{payments.map((payment) => <option key={payment.id} value={payment.id}>{payment.paymentNumber} · {payment.customer?.name} · TSh {Number(payment.amount).toLocaleString()}</option>)}</select><input aria-label="Refund amount" type="number" min="1" value={amount} onChange={(event) => setAmount(event.target.value)} className="border border-[#172B4D]/15 px-3 py-2.5 text-sm" /><select aria-label="Refund method" value={methodId} onChange={(event) => setMethodId(event.target.value)} className="border border-[#172B4D]/15 bg-white px-3 py-2.5 text-sm"><option value="">Select method</option>{methods.map((method) => <option key={method.id} value={method.id}>{method.name}</option>)}</select><button type="button" onClick={() => void createRefund()} className="flex items-center justify-center gap-2 bg-[#172B4D] px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.1em] text-white"><RotateCcw size={15} /> Record refund</button></div></section><section className="mt-8 space-y-3" aria-label="Refunds"><h2 className="text-xl font-semibold">Recent refunds</h2>{loading && <p className="bg-white p-5 text-sm text-[#172B4D]/55">Loading refunds...</p>}{!loading && refunds.length === 0 && <p className="bg-white p-5 text-sm text-[#172B4D]/55">No refunds found.</p>}{refunds.map((refund) => <article key={refund.id} className="flex items-center justify-between bg-white p-5"><div><p className="font-semibold">{refund.paymentNumber}</p><p className="mt-1 text-xs uppercase tracking-[0.12em] text-[#172B4D]/50">{refund.customer?.name} · {refund.status}</p></div><p className="font-semibold text-[#2563EB]">TSh {Math.abs(Number(refund.amount)).toLocaleString()}</p></article>)}</section></div></main></>;
}
