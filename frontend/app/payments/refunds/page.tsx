...
export default function RefundsPage() {
  ...
  return <><WorkspaceNavigation /><main className="min-h-screen bg-background px-4 py-5 text-foreground sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto max-w-6xl"><header className="flex flex-col justify-between gap-5 border-b border-border-default pb-7 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-primary">Finance desk</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.05em]">Refunds with a trace.</h1><p className="mt-2 text-sm text-foreground/55">Return funds against a recorded customer payment.</p></div><button type="button" onClick={() => void loadData()} className="flex items-center gap-2 border border-border-default px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.12em]"><RefreshCw size={15} /> Refresh</button></header>{error && <div role="alert" className="mt-6 flex items-center gap-3 border border-border-default bg-primary/8 px-4 py-3 text-sm text-muted-foreground"><AlertCircle size={18} /> {error}</div>}{message && <div role="status" className="mt-6 border border-border-default bg-status-success-surface px-4 py-3 text-sm text-muted-foreground">{message}</div>}<section className="mt-8 bg-card p-6"><h2 className="text-xl font-semibold">New refund</h2><div className="mt-4 grid gap-3 sm:grid-cols-4"><select aria-label="Original payment" value={paymentId} onChange={(event) => setPaymentId(event.target.value)} className="border border-border-default bg-card px-3 py-2.5 text-sm"><option value="">Select payment</option>{payments.map((payment) => <option key={payment.id} value={payment.id}>{payment.paymentNumber} · {payment.customer?.name} · TSh {Number(payment.amount).toLocaleString()}</option>)}</select><input aria-label="Refund amount" type="number" min="1" value={amount} onChange={(event) => setAmount(event.target.value)} className="border border-border-default px-3 py-2.5 text-sm" /><select aria-label="Refund method" value={methodId} onChange={(event) => setMethodId(event.target.value)} className="border border-border-default bg-card px-3 py-2.5 text-sm"><option value="">Select method</option>{methods.map((method) => <option key={method.id} value={method.id}>{method.name}</option>)}</select><button type="button" onClick={() => void createRefund()} className="flex items-center justify-center gap-2 bg-primary px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.1em] text-white"><RotateCcw size={15} /> Record refund</button></div></section><section className="mt-8 space-y-3" aria-label="Refunds"><h2 className="text-xl font-semibold">Recent refunds</h2>{loading && <p className="bg-card p-5 text-sm text-foreground/55">Loading refunds...</p>}{!loading && refunds.length === 0 && <p className="bg-card p-5 text-sm text-foreground/55">No refunds found.</p>}{refunds.map((refund) => <article key={refund.id} className="flex items-center justify-between bg-card p-5"><div><p className="font-semibold">{refund.paymentNumber}</p><p className="mt-1 text-xs uppercase tracking-[0.12em] text-foreground/50">{refund.customer?.name} · {refund.status}</p></div><p className="font-semibold text-primary">TSh {Math.abs(Number(refund.amount)).toLocaleString()}</p></article>)}</section></div></main></>;
}
