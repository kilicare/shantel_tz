...
export default function InvoicesPage() {
  ...
  return <><WorkspaceNavigation /><div className="mx-auto w-full min-w-0 max-w-7xl space-y-5 px-4 py-5 sm:px-6 sm:py-8 lg:px-8"><header className="flex flex-col gap-4 border-b border-[#17221f]/10 pb-5 sm:flex-row sm:items-end sm:justify-between"><div className="min-w-0"><p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-[#ad6742]">Sales desk</p><h1 className="mt-2 text-2xl font-semibold tracking-[-0.04em] text-[#17221f] sm:text-3xl">Invoices</h1><p className="mt-2 text-sm text-[#17221f]/55">Search, inspect and print customer invoices.</p></div>{canCreate ? <Button onClick={() => router.push("/invoices/new")} className="w-full sm:w-auto"><FilePlus2 className="size-4" /> New invoice</Button> : null}</header><ErrorAlert error={error?.message ?? null} onDismiss={() => void refetch()} />{loading ? <LoadingSpinner message="Loading invoices" /> : <div className="rounded-2xl border border-[#17221f]/10 bg-white p-2 shadow-sm sm:p-3"><DataTable columns={columns} data={invoices} searchFields={["invoiceNumber", "customer.name", "status"]} onRowClick={(row) => router.push(`/invoices/${row.id}`)} exportable={canExport} onExport={() => { void openAuthenticatedDocument(`/reports/export/sales-excel?startDate=${currentYear}-01-01&endDate=${currentYear}-12-31`, "sales-invoices.xlsx"); }} /></div>}</div></>;
}
