...
export default function PurchasingPage() {
  ...
  async function loadPurchasing() {
    try {
      setLoading(true);
      setError("");
      const [requisitionResponse, orderResponse, grnResponse, productResponse] =
        await Promise.all([
      ...
      setError(
        Array.isArray(messageValue)
          ? messageValue[0]
          : messageValue || "Purchasing data could not be loaded.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function createRequisition(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      setSaving(true);
      setError("");
      const response = await apiClient.post("/purchasing/requisitions", {
        items: [{ productId: form.productId, quantity: Number(form.quantity) }],
        notes: form.notes,
      });
      const created = unwrap(response);
      setMessage(
        `${created.requisitionNumber ?? "Requisition"} created successfully.`,
      );
      ...
      setError(
        Array.isArray(messageValue)
          ? messageValue[0]
          : messageValue || "Requisition could not be created.",
      );
    } finally {
      setSaving(false);
    }
