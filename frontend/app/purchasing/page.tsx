...
export default function PurchasingPage() {
  ...
  return <><WorkspaceNavigation /><main className="min-h-screen bg-[#F6F8FB] px-4 py-5 text-[#172B4D] sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto max-w-7xl"><header className="flex flex-col justify-between gap-5 border-b border-[#172B4D]/12 pb-7 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#2563EB]">Procurement desk</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.05em]">Move purchases into stock.</h1><p className="mt-2 text-sm text-[#172B4D]/55">Requisitions, purchase orders, and receiving in one controlled flow.</p></div><button type="button" onClick={() => void loadPurchasing()} className="flex items-center gap-2 border border-[#172B4D]/15 px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.12em] hover:bg-white"><RefreshCw size={15} /> Refresh data</button></header>{error && <div role="alert" className="mt-6 flex items-center gap-3 border border-[#2563EB]/30 bg-[#2563EB]/8 px-4 py-3 text-sm text-[#5B3A0F]"><AlertCircle size={18} /> {error}</div>}{message && <div role="status" className="mt-6 border border-[#16805C]/30 bg-[#16805C]/10 px-4 py-3 text-sm text-[#16805C]">{message}</div>}<section className="mt-8 grid gap-4 md:grid-cols-3">{lists.map(({ title, icon: Icon, items }) => <article key={title} className="bg-white p-5"><div className="flex items-center justify-between"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#172B4D]/48">{title}</p><Icon size={18} className="text-[#2563EB]" /></div><p className="mt-7 text-3xl font-semibold">{loading ? "..." : items.length}</p></article>)}</section><section className="mt-8 flex flex-wrap gap-2" aria-label="Purchasing actions">{canCreate && <button type="button" onClick={() => setShowForm(true)} className="flex items-center gap-2 bg-[#172B4D] px-4 py-3 text-xs font-semibold uppercase tracking-[0.1em] text-white"><Plus size={15} /> New requisition</button>}{canApprove && <button type="button" onClick={() => window.location.assign("/approvals")} className="border border-[#172B4D]/20 bg-white px-4 py-3 text-xs font-semibold uppercase tracking-[0.1em]">Review approvals</button>}{canPost && <button type="button" onClick={() => window.location.assign("/inventory")} className="border border-[#172B4D]/20 bg-white px-4 py-3 text-xs font-semibold uppercase tracking-[0.1em]">Post GRN</button>}</section>{showForm && <form onSubmit={createRequisition} className="mt-8 bg-[#172B4D] p-6 text-[#F6F8FB] sm:p-8"><div className="flex items-start justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#D4A72C]">Purchase request</p><h2 className="mt-2 text-2xl font-semibold">New requisition</h2></div><button type="button" onClick={() => setShowForm(false)} aria-label="Close requisition form"><X size={20} /></button></div><div className="mt-6 grid gap-4 sm:grid-cols-2"><label className="text-xs uppercase tracking-[0.12em] text-[#F6F8FB]/60">Product<select aria-label="Requisition product" value={form.productId} onChange={(event) => setForm({ ...form, productId: event.target.value })} className="mt-2 h-11 w-full bg-[#172B4D] text-sm normal-case tracking-normal outline-none"><option value="">Select product</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name} · {product.sku}</option>)}</select></label><label className="text-xs uppercase tracking-[0.12em] text-[#F6F8FB]/60">Quantity<input aria-label="Requisition quantity" type="number" min="1" value={form.quantity} onChange={(event) => setForm({ ...form, quantity: event.target.value })} className="mt-2 h-11 w-full border-b border-[#F6F8FB]/20 bg-transparent text-sm normal-case tracking-normal outline-none" /></label><label className="text-xs uppercase tracking-[0.12em] text-[#F6F8FB]/60 sm:col-span-2">Notes<input aria-label="Requisition notes" value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} className="mt-2 h-11 w-full border-b border-[#F6F8FB]/20 bg-transparent text-sm normal-case tracking-normal outline-none" /></label></div><button type="submit" disabled={saving} className="mt-6 bg-[#D4A72C] px-5 py-3 text-sm font-semibold text-[#172B4D] disabled:opacity-50">{saving ? "Creating..." : "Create requisition"}</button></form>}<section className="mt-8 grid gap-5 lg:grid-cols-3">{lists.map(({ title, items, number }) => <article key={title} className="bg-white p-6"><h2 className="text-lg font-semibold">{title}</h2><div className="mt-4 divide-y divide-[#172B4D]/10">{items.length ? items.slice(0, 8).map((item, index) => <div key={item.id ?? index} className="py-3"><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold">{(item as any)[number] ?? "Unnumbered document"}</p><span className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#2563EB]">{item.status ?? "UNKNOWN"}</span></div><p className="mt-1 text-xs text-[#172B4D]/45">{item.supplier?.name ?? "Supplier pending"}</p></div>) : <p className="py-7 text-sm text-[#172B4D]/50">No {title.toLowerCase()} found.</p>}</div></article>)}</section></div></main></>;
}
