...
export default function PurchaseReturnsPage() {
  ...
  return <><WorkspaceNavigation /><main className="min-h-screen bg-[#F6F8FB] px-4 py-5 text-[#172B4D] sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto max-w-6xl"><header className="flex flex-wrap items-end justify-between gap-5 border-b border-[#172B4D]/12 pb-7"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#2563EB]">Procurement control</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.05em]">Purchase returns.</h1><p className="mt-2 text-sm text-[#172B4D]/55">Return accepted stock with approval, inventory movement, and supplier balance evidence.</p></div><button type="button" onClick={() => void load()} className="flex items-center gap-2 border border-[#172B4D]/15 px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.12em]"><RefreshCw size={15} /> Refresh</button></header>
    {error && <div role="alert" className="mt-6 flex items-center gap-3 border border-[#C94A4A]/30 bg-[#FDECEC] px-4 py-3 text-sm text-[#C94A4A]"><AlertCircle size={18} /> {error}</div>}{notice && <div role="status" className="mt-6 border border-[#16805C]/30 bg-[#16805C]/10 px-4 py-3 text-sm text-[#16805C]">{notice}</div>}
    <form onSubmit={createReturn} className="mt-8 border border-[#172B4D]/10 bg-white p-5 sm:p-6"><div className="flex items-center gap-3"><PackageMinus className="text-[#2563EB]" size={20} /><h2 className="text-xl font-semibold">Create purchase return</h2></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><select aria-label="Return GRN" value={form.grnId} onChange={(event) => void chooseGRN(event.target.value)} className="h-11 border border-[#172B4D]/15 bg-white px-3 text-sm"><option value="">Select posted GRN</option>{grns.map((grn) => <option key={grn.id} value={grn.id}>{grn.grnNumber} · {grn.supplier?.name}</option>)}</select><select aria-label="Return product" value={form.productId} onChange={(event) => setForm({ ...form, productId: event.target.value })} className="h-11 border border-[#172B4D]/15 bg-white px-3 text-sm"><option value="">Select product</option>{selectedItems?.map((item) => <option key={item.productId} value={item.productId}>{item.product?.sku} · {item.product?.name} · accepted {item.acceptedQuantity}</option>)}</select><input aria-label="Return quantity" type="number" min="0.01" step="0.01" value={form.quantity} onChange={(event) => setForm({ ...form, quantity: event.target.value })} className="h-11 border border-[#172B4D]/15 px-3 text-sm" placeholder="Quantity" /><input aria-label="Return reason" value={form.reason} onChange={(event) => setForm({ ...form, reason: event.target.value })} className="h-11 border border-[#172B4D]/15 px-3 text-sm" placeholder="Reason" /><button type="submit" className="flex h-11 items-center justify-center gap-2 bg-[#172B4D] px-4 text-xs font-semibold uppercase tracking-[0.1em] text-white"><Send size={15} /> Submit return</button></div></form>
    <section className="mt-8 space-y-3"><h2 className="text-xl font-semibold">Return register</h2>{loading && <p className="bg-white p-5 text-sm text-[#172B4D]/55">Loading returns...</p>}{!loading && returns.length === 0 && <p className="bg-white p-5 text-sm text-[#172B4D]/55">No purchase returns found.</p>}{returns.map((record) => <article key={record.id} className="flex flex-col gap-4 bg-white p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">{record.returnNumber}</p><p className="mt-1 text-xs uppercase tracking-[0.1em] text-[#172B4D]/50">{record.status} · {record.supplier?.name} · {record.grn?.grnNumber}</p></div><div className="flex flex-wrap items-center gap-2">{record.status === "DRAFT" && <button type="button" onClick={() => void transition(record, "approve")} className="flex items-center gap-2 bg-[#172B4D] px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.1em] text-white"><Check size={15} /> Approve</button>}{record.status === "SUBMITTED" && <><select aria-label={`Posting location for ${record.returnNumber}`} value={form.locationId} onChange={(event) => setForm({ ...form, locationId: event.target.value })} className="border border-[#172B4D]/15 bg-white px-3 py-2.5 text-sm"><option value="">Select location</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select><button type="button" onClick={() => void transition(record, "post")} className="flex items-center gap-2 border border-[#172B4D]/20 px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.1em]"><Send size={15} /> Post</button></>}</div></article>)}</section>
  </div></main></>;
}
