      ...
      <main className="min-h-screen bg-background px-4 py-5 text-foreground sm:px-6 sm:py-8 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <header className="flex flex-col justify-between gap-5 border-b border-border-default pb-7 sm:flex-row sm:items-end">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-primary">Returns control</p>
              <h1 className="mt-2 text-4xl font-semibold tracking-[-0.05em]">Return goods with a trail.</h1>
              <p className="mt-2 text-sm text-foreground/55">Create, approve, and post customer returns.</p>
            </div>
            <button type="button" onClick={() => void loadData()} className="flex items-center gap-2 border border-border-default px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.12em]"><RefreshCw size={15} /> Refresh</button>
          </header>
          ...
          <section className="mt-8 bg-card p-6">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-4">
              <select aria-label="Return invoice" value={invoiceId} onChange={(event) => void selectInvoice(event.target.value)} className="border border-border-default bg-card px-3 py-2.5 text-sm"><option value="">Select invoice</option>{invoices.map((invoice) => <option key={invoice.id} value={invoice.id}>{invoice.invoiceNumber} · {invoice.customer?.name ?? "Customer"}</option>)}</select>
              <select aria-label="Return product" value={productId} onChange={(event) => setProductId(event.target.value)} className="border border-border-default bg-card px-3 py-2.5 text-sm"><option value="">Select product</option>{invoiceItems.map((item) => <option key={item.productId} value={item.productId}>{item.product?.name ?? "Product"}</option>)}</select>
              <input aria-label="Return quantity" type="number" min="1" value={quantity} onChange={(event) => setQuantity(event.target.value)} className="border border-border-default px-3 py-2.5 text-sm" />
              <input aria-label="Return reason" value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Reason" className="border border-border-default px-3 py-2.5 text-sm" />
              <button type="button" onClick={() => void createReturn()} className="flex items-center justify-center gap-2 bg-primary px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.1em] text-white"><PackagePlus size={15} /> Create return</button>
            </div>
          </section>
          <section className="mt-8 space-y-3" aria-label="Sales returns">
            <h2 className="text-xl font-semibold">Sales returns</h2>
            {loading && <p className="bg-card p-5 text-sm text-foreground/55">Loading returns...</p>}
            {!loading && returns.length === 0 && <p className="bg-card p-5 text-sm text-foreground/55">No sales returns found.</p>}
            {returns.map((item) => <article key={item.id} className="flex flex-col gap-3 bg-card p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-semibold">{item.returnNumber ?? "Sales return"}</p><p className="mt-1 text-xs uppercase tracking-[0.12em] text-foreground/50">{item.status} · {item.invoice?.invoiceNumber ?? "Invoice"} · Refund TSh {Number(item.refundAmount ?? 0).toLocaleString()}</p>{item.reason && <p className="mt-1 text-xs text-foreground/55">{item.reason}</p>}</div>{item.status === "DRAFT" && <button type="button" onClick={() => void approveReturn(item)} className="flex items-center gap-2 bg-primary px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.1em] text-white"><Check size={15} /> Approve</button>}{item.status === "SUBMITTED" && <div className="flex flex-wrap gap-2"><select aria-label={`Return posting location for ${item.returnNumber ?? "return"}`} value={returnLocations[item.id] ?? ""} onChange={(event) => setReturnLocations((current) => ({ ...current, [item.id]: event.target.value }))} className="border border-border-default bg-card px-3 py-2.5 text-sm"><option value="">Select location</option>{locations.map((location) => <option key={location.id} value={location.id}>{location.name}</option>)}</select><button type="button" onClick={() => void postReturn(item)} className="flex items-center gap-2 border border-border-default px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.1em]"><Send size={15} /> Post</button></div>}</article>)}
          </section>
        </div>
      </main>
    </>
