...
export default function SalesReportsPage() {
  ...
  return <><WorkspaceNavigation /><main className="min-h-screen bg-background px-4 py-5 text-foreground sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto w-full max-w-7xl"><header className="flex flex-col justify-between gap-5 border-b border-border-default pb-7 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-primary">Sales intelligence</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.05em]">Sales report</h1><p className="mt-2 text-sm text-foreground/55">Posted sales, paid totals, balances, and invoice detail by date range.</p></div><button type="button" onClick={() => void load()} className="flex items-center gap-2 border border-border-default px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.12em]"><RefreshCw size={15} /> Refresh</button></header>{error && <div className="mt-6"><AlertBanner>{error}</AlertBanner></div>}<form onSubmit={load} className="mt-8 flex flex-wrap items-end gap-3"><label className="text-xs font-semibold uppercase tracking-[0.12em]">From<input type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} className="mt-2 block h-11 border border-border-default bg-card px-3 text-sm normal-case tracking-normal" /></label><label className="text-xs font-semibold uppercase tracking-[0.12em]">To<input type="date" value={endDate} onChange={(event) => setEndDate(event.target.value)} className="mt-2 block h-11 border border-border-default bg-card px-3 text-sm normal-case tracking-normal" /></label><button type="submit" className="h-11 bg-primary px-4 text-xs font-semibold uppercase tracking-[0.12em] text-white">Apply dates</button>{(["csv", "excel", "pdf"] as const).map((format) => <button key={format} type="button" onClick={() => void exportReport(format)} className="flex h-11 items-center gap-2 border border-border-default px-3 text-xs font-semibold uppercase tracking-[0.1em]"><Download size={14} />{format}</button>)}</form>{loading ? <LoadingState message="Loading sales report..." /> : !report || report.invoices.length === 0 ? <div className="mt-8"><EmptyState title="No posted sales in this period" description="Try a wider date range." /></div> : <><section className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><ShantelCard className="p-5"><p className="text-xs text-foreground/55">Invoices</p><p className="mt-2 text-2xl font-semibold">{report.summary.invoiceCount}</p></ShantelCard><ShantelCard className="p-5"><p className="text-xs text-foreground/55">Revenue</p><p className="mt-2 text-2xl font-semibold">{money.format(report.summary.totalRevenue)}</p></ShantelCard><ShantelCard className="p-5"><p className="text-xs text-foreground/55">Paid</p><p className="mt-2 text-2xl font-semibold">{money.format(report.summary.totalPaid)}</p></ShantelCard><ShantelCard className="p-5"><p className="text-xs text-foreground/55">Outstanding</p><p className="mt-2 text-2xl font-semibold">{money.format(report.summary.totalOutstanding)}</p></ShantelCard></section><section className="mt-8 bg-card p-6"><h2 className="text-2xl font-semibold">Posted invoice detail</h2><div className="mt-5 divide-y divide-[#172B4D]/10">{report.invoices.map((invoice) => <div key={invoice.invoiceNumber} className="grid gap-2 py-4 sm:grid-cols-[1.2fr_1fr_1fr_1fr] sm:items-center"><div><p className="font-semibold">{invoice.invoiceNumber}</p><p className="text-xs text-foreground/55">{invoice.customer} · {invoice.itemCount} items</p></div><p className="text-sm">{new Date(invoice.invoiceDate).toLocaleDateString()}</p><p className="text-sm">{money.format(invoice.totalAmount)}</p><p className="text-sm">Balance {money.format(invoice.balance)}</p></div>)}</div></section></>}</div></main></>;
}
