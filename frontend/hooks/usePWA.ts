...
export function usePWA() {
  ...
  useEffect(() => { const sync = () => { setIsInstalled(window.matchMedia("(display-mode: standalone)").matches || (window.navigator as Navigator & { standalone?: boolean }).standalone === true); setIsOnline(navigator.onLine); }; const online = () => setIsOnline(true); const offline = () => setIsOnline(false); const frame = requestAnimationFrame(sync); window.addEventListener("online", online); window.addEventListener("offline", offline); return () => { cancelAnimationFrame(frame); window.removeEventListener("online", online); window.removeEventListener("offline", offline); }; }, []);
