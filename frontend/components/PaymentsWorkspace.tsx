...
export function PaymentsWorkspace() {
  ...
  return <><WorkspaceNavigation /><main className="min-h-screen bg-[#f4f1ec] px-4 py-5 text-[#17221f] sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto w-full min-w-0 max-w-7xl"><header className="flex flex-col justify-between gap-5 border-b border-[#17221f]/12 pb-7 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#ad6742]">Finance desk</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.05em]">Payments</h1><p className="mt-2 text-sm text-[#17221f]/55">Record customer payments with traceable method and reference details.</p></div><div className="flex gap-2"><button type="button" onClick={() => void load()} className="flex items-center gap-2 border border-[#17221f]/15 px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.12em]"><RefreshCw size={15} /> Refresh</button>{permissions.includes("payments.record") && <button type="button" onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 bg-[#17221f] px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.12em] text-white"><CreditCard size={15} /> {showForm ? "Close" : "Record payment"}</button>}</div></header>{error && <div role="alert" className="mt-6 flex items-center gap-3 border border-[#ad6742]/30 bg-[#ad6742]/8 px-4 py-3 text-sm text-[#8a4931]"><AlertCircle size={18} /> {error}</div>}{message && <div role="status" className="mt-6 border border-[#567b68]/30 bg-[#567b68]/10 px-4 py-3 text-sm text-[#365b4a]">{message}</div>}{showForm && <form onSubmit={record} className="mt-8 grid gap-4 bg-[#17221f] p-6 text-[#f4f1ec] sm:grid-cols-2"><h2 className="text-xl font-semibold sm:col-span-2">Record payment</h2><select required aria-label="Payment customer" value={form.customerId} onChange={(event) => setForm({ ...form, customerId: event.target.value })} className="h-11 bg-[#f4f1ec] px-2 text-sm text-[#17221f]"><option value="">Select customer</option>{customers.map((customer) => <option key={customer.id} value={customer.id}>{customer.name}</option>)}</select><input required aria-label="Payment amount" type="number" min="0.01" step="0.01" placeholder="Amount" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} className="h-11 bg-[#f4f1ec] px-2 text-sm text-[#17221f]" /><select required aria-label="Payment method" value={form.paymentMethodId} onChange={(event) => setForm({ ...form, paymentMethodId: event.target.value })} className="h-11 bg-[#f4f1ec] px-2 text-sm text-[#17221f]"><option value="">Select payment method</option>{methods.map((method) => <option key={method.id} value={method.id}>{method.name}</option>)}</select><input aria-label="Payment reference" placeholder="Reference" value={form.transactionReference} onChange={(event) => setForm({ ...form, transactionReference: event.target.value })} className="h-11 bg-[#f4f1ec] px-2 text-sm text-[#17221f]" /><button type="submit" className="bg-[#e8a36b] px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.1em] text-[#17221f] sm:col-span-2">Record payment</button></form>}<section className="mt-8 bg-white p-6"><div className="flex items-end justify-between"><h2 className="text-2xl font-semibold">Recent payments</h2><span className="text-xs text-[#17221f]/45">{payments.length} records</span></div>{loading ? <p className="py-10 text-sm text-[#17221f]/50">Loading payments...</p> : <div className="mt-4 divide-y divide-[#17221f]/10">{payments.map((payment) => <div key={payment.id} className="flex items-center justify-between py-4"><div><p className="text-sm font-semibold">{payment.paymentNumber}</p><p className="text-xs text-[#17221f]/45">{payment.customer?.name ?? "Customer"}</p></div><div className="text-right"><p className="text-sm font-semibold">TSh {payment.amount}</p><StatusBadge status={payment.status ?? "PENDING"} className="mt-1" /></div></div>)}</div>}</section><PaymentMethodWorkspace methods={methods} onChanged={load} /></div></main></>;
}
