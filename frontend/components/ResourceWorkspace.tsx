...
export function ResourceWorkspace({ eyebrow, title, description, endpoint, emptyLabel, createPermission, createLabel = "New record" }: ResourceWorkspaceProps) {
  ...
  return <><WorkspaceNavigation /><main className="min-h-screen bg-[#F6F8FB] px-4 py-5 text-[#172B4D] sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto w-full min-w-0 max-w-7xl"><header className="flex flex-col justify-between gap-5 border-b border-[#172B4D]/12 pb-7 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#2563EB]">{eyebrow}</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.05em]">{title}</h1><p className="mt-2 text-sm text-[#172B4D]/55">{description}</p></div><div className="flex gap-2"><button type="button" onClick={() => void load()} className="flex items-center gap-2 border border-[#172B4D]/15 px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.12em] hover:bg-white"><RefreshCw size={15} /> Refresh</button>{canCreate && <button type="button" className="flex items-center gap-2 bg-[#172B4D] px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.12em] text-white"><Plus size={15} /> {createLabel}</button>}</div></header>{error && <div className="mt-6"><AlertBanner>{error}</AlertBanner></div>}<ShantelCard className="mt-8 p-4 sm:p-6"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#2563EB]">Directory</p><h2 className="mt-2 text-2xl font-semibold">{title}</h2></div><span className="text-xs text-[#172B4D]/45">{filtered.length} records</span></div><div className="relative mt-5"><Search size={17} className="absolute left-0 top-3 text-[#172B4D]/35" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={`Search ${title.toLowerCase()}`} className="h-11 w-full border-b border-[#172B4D]/15 bg-transparent pl-7 text-sm outline-none focus:border-[#2563EB]" /></div>{loading ? <LoadingState message={`Loading ${title.toLowerCase()}...`} /> : filtered.length ? <div className="mt-4 divide-y divide-[#172B4D]/10">{filtered.slice(0, 100).map((record, index) => <div key={record.id ?? index} className="py-4"><p className="text-sm font-semibold">{record.name ?? record.product?.name ?? record.supplier?.name ?? record.email ?? record.number ?? record.invoiceNumber ?? record.id ?? "Record"}</p><p className="mt-1 line-clamp-2 text-xs text-[#172B4D]/45">{record.status ?? record.code ?? record.phone ?? record.description ?? "Operational record"}</p></div>)}</div> : <EmptyState title={emptyLabel} />}</ShantelCard></div></main></>;
}
