...
export function ResourceWorkspace({ eyebrow, title, description, endpoint, emptyLabel, createPermission, createLabel = "New record" }: ResourceWorkspaceProps) {
  ...
  useEffect(() => { try { setPermissions(JSON.parse(sessionStorage.getItem("shantel_user") ?? "null")?.permissions ?? []); } catch { setPermissions([]); } void load(); }, []);
  ...
  return <><WorkspaceNavigation /><main className="min-h-screen bg-background px-4 py-5 text-foreground sm:px-6 sm:py-8 lg:px-8"><div className="mx-auto w-full min-w-0 max-w-7xl"><header className="flex flex-col justify-between gap-5 border-b border-border-default pb-7 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.24em] text-primary">{eyebrow}</p><h1 className="mt-2 text-4xl font-semibold tracking-[-0.05em]">{title}</h1><p className="mt-2 text-sm text-foreground/55">{description}</p></div><div className="flex gap-2"><button type="button" onClick={() => void load()} className="flex items-center gap-2 border border-border-default px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.12em] hover:bg-card"><RefreshCw size={15} /> Refresh</button>{canCreate && <button type="button" className="flex items-center gap-2 bg-primary px-4 py-2.5 text-xs font-semibold uppercase tracking-[0.12em] text-white"><Plus size={15} /> {createLabel}</button>}</div></header>{error && <div className="mt-6"><AlertBanner>{error}</AlertBanner></div>}<ShantelCard className="mt-8 p-4 sm:p-6"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-primary">Directory</p><h2 className="mt-2 text-2xl font-semibold">{title}</h2></div><span className="text-xs text-foreground/45">{filtered.length} records</span></div><div className="relative mt-5"><Search size={17} className="absolute left-0 top-3 text-foreground/35" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder={`Search ${title.toLowerCase()}`} className="h-11 w-full border-b border-border-default bg-transparent pl-7 text-sm outline-none focus:border-primary" /></div>{loading ? <LoadingState message={`Loading ${title.toLowerCase()}...`} /> : filtered.length ? <div className="mt-4 divide-y divide-border-default">{filtered.slice(0, 100).map((record, index) => <div key={record.id ?? index} className="py-4"><p className="text-sm font-semibold">{record.name ?? record.product?.name ?? record.supplier?.name ?? record.email ?? record.number ?? record.invoiceNumber ?? record.id ?? "Record"}</p><p className="mt-1 line-clamp-2 text-xs text-foreground/45">{record.status ?? record.code ?? record.phone ?? record.description ?? "Operational record"}</p></div>)}</div> : <EmptyState title={emptyLabel} />}</ShantelCard></div></main></>;
}
