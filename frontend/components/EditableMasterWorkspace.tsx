...
export function EditableMasterWorkspace({ kind }: { kind: Kind }) {
  ...
  async function load() {
    try {
      ...
      const records = unwrap(
        await apiClient.get(`${current.endpoint}?page=1&limit=100`),
      );
      if (kind === "customers") {
        const withBalances = await Promise.all(
          records.map(async (item: Item) => ({
            ...item,
            balance:
              (await apiClient.get(`/customers/${item.id}/balance`)).data?.data
                ?.totalBalance ?? 0,
          })),
        );
        setItems(withBalances);
      } else if (kind === "suppliers") {
        const withBalances = await Promise.all(
          records.map(async (item: Item) => ({
            ...item,
            balance:
              (await apiClient.get(`/suppliers/${item.id}/balance`)).data?.data
                ?.totalBalance ?? 0,
          })),
        );
        setItems(withBalances);
      } else setItems(records);
    } catch (requestError: any) {
      const value = requestError?.response?.data?.message;
      setError(
        Array.isArray(value)
          ? value[0]
          : value || `${current.title} could not be loaded.`,
      );
    } finally {
      setLoading(false);
    }
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      const response = editingId
        ? await apiClient.patch(`${current.endpoint}/${editingId}`, body)
      ...
      setError(
        Array.isArray(value)
          ? value[0]
          : value || `${current.singular} could not be saved.`,
      );
    }
  }
  async function deactivate(item: Item) {
    try {
      await apiClient.patch(`${current.endpoint}/${item.id}/deactivate`);
      setMessage(`${item.name} deactivated successfully.`);
      await load();
    } catch (requestError: any) {
      const value = requestError?.response?.data?.message;
      setError(
        Array.isArray(value)
          ? value[0]
          : value || `${item.name} could not be deactivated.`,
      );
    }
  }
  ...
  const canEdit = permissions.includes(`${kind}.edit`);
      ...
      <main className="min-h-screen bg-background px-4 py-5 text-foreground sm:px-6 sm:py-8 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <header className="flex flex-col justify-between gap-5 border-b border-border-default pb-8 sm:flex-row sm:items-end">
            ...
            <div className="flex gap-2">
              ...
              {canCreate && (
                ...
                >
                  <Plus size={16} /> New {current.singular}
                </button>
              )}
            </div>
          </header>
          ...
          {showForm && (
            <form
              onSubmit={save}
              className="mt-8 grid gap-4 bg-brand-primary p-6 text-primary-foreground sm:grid-cols-2"
            >
              <h2 className="text-h3 font-semibold sm:col-span-2">
                {editingId
              ...
              <input
                ...
                className="h-11 border-b border-border-default bg-transparent text-body outline-none"
              />
              {kind === "locations" ? (
                <>
                  <input
                  ...
                  >
                    ...
                    <option value="PROJECT_STORE">Project store</option>
                  ...
                  {editingId && (
                    <select
                      ...
                      className="h-11 bg-background px-2 text-body text-foreground"
                    >
              ...
              <input
                ...
                className="h-11 border-b border-border-default/25 bg-transparent text-body outline-none"
              />
              <button
                type="submit"
                className="bg-surface px-4 py-2.5 text-label font-semibold text-foreground sm:col-span-2"
              >
                {editingId
                  ? "Save changes"
                  : `Create ${current.singular.toLowerCase()}`}
              </button>
            </form>
          )}
          <section className="mt-8 bg-surface p-6">
            <div className="flex items-end justify-between">
              <h2 className="text-h2 font-semibold">{current.title}</h2>
            ...
            <input
              ...
              className="mt-5 h-11 w-full border-b border-border-default bg-transparent text-body outline-none"
            />
            {loading ? (
              <p className="py-10 text-body text-foreground/50">Loading...</p>
            ) : (
