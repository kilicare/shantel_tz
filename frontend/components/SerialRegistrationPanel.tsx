...
export function SerialRegistrationPanel() {
  ...
  return <section className="mt-8 bg-[#17221f] p-6 text-[#f4f1ec]"><div className="flex items-center gap-3"><ScanLine size={18} className="text-[#e8a36b]" /><div><p className="text-xs uppercase tracking-[0.2em] text-[#e8a36b]">Serial tracking</p><h2 className="mt-1 text-xl font-semibold">Register serial number</h2></div></div>{error && <p role="alert" className="mt-4 flex items-center gap-2 text-sm text-[#f3a080]"><AlertCircle size={16} /> {error}</p>}{message && <p role="status" className="mt-4 flex items-center gap-2 text-sm text-[#b9ddc8]"><Check size={16} /> {message}</p>}<form onSubmit={register} className="mt-5 grid gap-3 sm:grid-cols-2"><select required aria-label="Serial product" value={productId} onChange={(event) => setProductId(event.target.value)} className="h-11 bg-[#f4f1ec] px-2 text-sm text-[#17221f]"><option value="">Select product</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name} · {product.sku}</option>)}</select><input required aria-label="Serial number" placeholder="Serial number" value={serialNumber} onChange={(event) => setSerialNumber(event.target.value)} className="h-11 border-b border-white/25 bg-transparent text-sm outline-none" /><button type="submit" className="bg-[#e8a36b] px-4 py-2.5 text-xs font-semibold text-[#17221f] sm:col-span-2">Register serial</button></form></section>;
}
