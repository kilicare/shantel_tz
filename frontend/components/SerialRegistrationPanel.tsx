...
export function SerialRegistrationPanel() {
  ...
  useEffect(() => {
    void apiClient
      ...
      .then((response) => setProducts(unwrap(response)))
  ...
  async function register(event: FormEvent) {
    ...
    try {
      ...
      const response = await apiClient.post(
        "/projects/serial-numbers/register",
        { productId, serialNumber },
      );
      setMessage(`${unwrap(response).serialNumber} registered successfully.`);
      setSerialNumber("");
      ...
      setError(
        Array.isArray(value)
          ? value[0]
          : value || "Serial number could not be registered.",
      );
    }
  }
  return (
